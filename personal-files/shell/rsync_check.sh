...
then
        ...
        if [ "$filedic" = "" ]
        then
                echo "$1 has not been backup" && exit
        else
...
then
        ...
        then
                ...
                then
                        fpom=$(find $filedic -name "*.pom" -type f)
                        if [ "$fpom"x = ""x ]
                        then
                                echo "the *.pom file is not found in the version"
                        else
                                echo "the file has been backup failed ,and the source file size is $filesize,now is $filesize2"
                        fi      
                fi
        fi
fi
